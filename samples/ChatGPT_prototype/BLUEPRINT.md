...
## Design spec (light, documentation-like)

- Layout: single column, `max-width: 72rem` list pages / `65ch` text pages, generous whitespace.
- Type: system font stack (`system-ui, ...`); base 16–17px, line-height 1.6.
- Colors (CSS custom properties in `:root`): background `#ffffff`, text `#1a1f24`, muted `#5b6670`, accent `#2f6f4f` (restrained military green, used for links/buttons/badges only), borders `#e3e7ea`, card bg `#f7f9fa`. No dark mode in v1 (keep CSS simple); acceptable to add later.
- Buttons: solid accent bg, white text, subtle radius (6px). Cards: 1px border, 8px radius, no shadows.
- No hero images, no decorative graphics in v1. The screenshots are the visuals.
